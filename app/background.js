...
var Consts = {
  xClientHeader:
    "5a8238ab-1819-4f7f-a750-f23264719a2d-HabiticaPomodoroSiteKeeper",
  ...
  RewardTemplate: {
    ...
    value: 0,
    notes:
      "Reward utilized by Habitica SiteKeeper." +
  ...
  PomodoroHabitTemplate: {
    ...
    notes:
      "Habit utilized by Habitica SiteKeeper. " +
...
function UserSettings(copyFrom) {
  ...
  this.pomodoroEndSoundVolume = copyFrom
    ? copyFrom.pomodoroEndSoundVolume
    : 0.5;
...
function RemoveBlockedSite(site) {
  ...
  } else delete Vars.UserData.BlockedSites[site];
}

function AddBlockedSite(hostname, cost, passDuration, passExpiry) {
  Vars.UserData.BlockedSites[hostname] = new BlockedSite(
    hostname,
    cost,
    passDuration,
    passExpiry
...
function checkBlockedUrl(siteUrl) {
  ...
  var freePass =
    (Vars.UserData.BreakFreePass && Vars.onBreak && Vars.TimerRunnig) ||
    ((Vars.UserData.VacationMode || isFreePassTimeNow()) &&
      (!Vars.TimerRunnig || Vars.onBreak));
...
function mainSiteBlockFunction(tab) {
  if (!Vars.TimerRunnig || Vars.onBreak) {
    ...
    var passDurationMiliSec =
      GetSitePassDuration(GetBlockedSite(siteUrl.hostname)) * 60 * 1000;
    setTimeout(
      function (arg) {
...
function payToPassOverlay(tab, siteData) {
  ...
  chrome.tabs.insertCSS({
    code:
      `
        .payToPass:after { background-image:url("` +
      imageURLPayToPass +
      `"); }
        .payToPass::before {background-color:rgba(0,0,0,` +
      opacity +
      `)!important}`,
  });
      ...
      chrome.tabs.executeScript(tab.id, {
        code:
          `
            ...
            document.getElementById("SitekeeperOverlay").setAttribute("data-html","You're trying to Access ` +
          siteData.hostname +
          `\\n Pay ` +
          siteData.cost +
          ` Gold to access for ` +
...
function cantAffordOverlay(tab, siteData) {
  ...
  chrome.tabs.insertCSS({
    code:
      `
        .noPass:after { background-image:url("` +
      imageURLNoPass +
      `"); }
        .noPass::before {background-color:rgba(0,0,0,` +
      opacity +
      `)}`,
  });
      ...
      chrome.tabs.executeScript(tab.id, {
        code:
          `
            ...
            document.getElementById("SitekeeperOverlay").setAttribute("data-html","You can't afford to visit ` +
          siteData.hostname +
          `\\n You shall not pass! ");
...
function callAPI(method, route, postData) {
  ...
  var serverUrl =
    Vars.UserData.developerServerUrl && Vars.UserData.developerServerUrl !== ""
      ? Vars.UserData.developerServerUrl
      : Consts.serverUrl;
  return callHabiticaAPI(
    serverUrl + route,
    Consts.xClientHeader,
    Vars.UserData.Credentials,
    method,
    postData
  );
}
  ...
  if (xhr.status == 401) {
    ...
    if (!silent) {
      chrome.notifications.create(
        Consts.NotificationId,
        {
          type: "basic",
          iconUrl: "img/icon.png",
          title: "Habitica Connection Error",
          message:
            "The service might be temporarily unavailable. Contact the developer if it persists. Error =" +
            xhr.status,
        },
        function () {}
      );
...
function FetchHabiticaData(skipTasks) {
  ...
  if (!skipTasks) {
    ...
    if (!Vars.UserData.CustomPomodoroTask) {
      ...
      if (
        tasksObj &&
        tasksObj.data["alias"] == Consts.PomodoroHabitTemplate.alias
      ) {
    ...
    if (!Vars.UserData.CustomSetTask) {
      ...
      if (
        tasksObj &&
        tasksObj.data["alias"] == Consts.PomodoroSetHabitTemplate.alias
      ) {
  ...
  //Popup communication
    ...
    if (request.msg === "get_data") {
      ...
      if (request.args) {
        ...
        response = {
          result: executeFunctionByName(request.functionName, ...request.args),
          complete: true,
        };
      } else {
        response = {
          result: executeFunctionByName(request.functionName),
          complete: true,
        };
      }
    }
...
function ScoreHabit(habitId, direction) {
  var p = JSON.parse(
    callAPI("POST", "/tasks/" + habitId + "/score/" + direction)
  );
  if (p.success != true) {
    return {
...
function duringPomodoro() {
  ...
  var timeString =
    BROWSER === "Mozilla Firefox" ? shortTimeString(Vars.Timer) : Vars.Timer;
  chrome.browserAction.setBadgeText({
    text: timeString,
  });
  ...
  if (currentAmbientAudio instanceof Audio && currentAmbientAudio.paused) {
    playSound(
      Vars.UserData.ambientSound,
      Vars.UserData.ambientSoundVolume,
      true
    );
  }
}
...
function increasePomodorosToday() {
  ...
  if (todaysData) {
    setTodaysHistogram(
      todaysData.pomodoros + 1,
      todaysData.minutes + Vars.UserData.PomoDurationMins
    );
  } else {
...
function pomodoroEnds() {
  ...
  var msg =
    ...
    "\n" +
    "You have done " +
    Vars.Histogram[getDate()].pomodoros +
    " today!"; //default msg if habit not enabled
  var setComplete = Vars.PomoSetCounter >= Vars.UserData.PomoSetNum - 1;
  //If Pomodoro / Pomodoro Set Habit + is enabled
  if (
    Vars.UserData.PomoHabitPlus ||
    (setComplete && Vars.UserData.PomoSetHabitPlus)
  ) {
    FetchHabiticaData(true);
    var result =
      setComplete && Vars.UserData.PomoSetHabitPlus
        ? ScoreHabit(Vars.PomodoroSetTaskId, "up")
        : ScoreHabit(Vars.PomodoroTaskId, "up");
    if (!result.error) {
      var deltaGold = (result.gp - Vars.Monies).toFixed(2);
      var deltaExp = (result.exp - Vars.Exp).toFixed(2);
      var expText =
        deltaExp < 0 ? "You leveled up!" : "You Earned Exp: +" + deltaExp;
  ...
  chrome.extension.getBackgroundPage().alert("Take a break! 🎉");
  //notify
  notify(title, msg);

  //play sound
  playSound(
    Vars.UserData.pomodoroEndSound,
    Vars.UserData.pomodoroEndSoundVolume,
    false
  );
}
...
function breakEnds() {
  ...
  chrome.extension.getBackgroundPage().alert("Break's over. Go get it! 😤");
  //notify
  notify("Time's Up", msg);
  //play sound
  playSound(
    Vars.UserData.breakEndSound,
    Vars.UserData.breakEndSoundVolume,
    false
  );
}
...
function startBreakExtension(duration) {
  ...
  Vars.onBreak = true;
  var endFunc = Vars.UserData.ResetPomoAfterBreak
    ? () => {
        pomodoroInterupted(true);
      }
    : pomodoroInterupted;
  ...
  if (Vars.UserData.BreakExtentionNotify) {
    notifyHabitica(
      "Back to work! " +
        secondsToTimeString(Vars.UserData.BreakExtention * 60) +
        " minutes left for Break Extension."
    );
  }
}
...
function pomodoroInterupted(breakPomoStreak) {
  ...
  var failedBreakExtension =
    Vars.UserData.BreakExtentionFails && Vars.onBreakExtension;
  var breakExtensionZero =
    !Vars.UserData.BreakExtentionFails && Vars.UserData.BreakExtention == 0;
  if (breakPomoStreak) {
    pomoReset();
  } else {
    pauseTimer();
...
function blockSiteOverlay(tab) {
  ...
  if (GetBlockedSite(url.hostname) && !isInWhiteList(url)) {
    chrome.tabs.executeScript({
      code:
        `
            ...
            document.body.setAttribute('data-html',"` +
        message +
        `");            
            `,
    });
    ...
    chrome.tabs.insertCSS({
      code:
        `
            .blockedSite:after {background-image:url("` +
        imageURL +
        `");}
            .blockedSite:before{background-color:rgba(0,0,0,` +
        opacity +
        `)}
            `,
    });
...
function playSound(soundFileName, volume, loop) {
  if (soundFileName != "None") {
    var myAudio =
      new Audio(chrome.runtime.getURL("audio/" + soundFileName)) || false;
    if (myAudio) {
